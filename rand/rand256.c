...
static uint64_t xorshift256_next(uint64_t state[4], int *p) {
	...
	uint64_t res = s0+s1;
	//s1 ^= s1 << 30; // a
	//state[*p] = s1 ^ s0 ^ (s1 >> 1) ^ (s0 >> 2); // b,c
	//s1 ^= s1 << 17; // a
	//state[*p] = s1 ^ s0 ^ (s1 >> 23) ^ (s0 >> 1); // b,c
	//s1 ^= s1 << 31; // a
	//state[*p] = s1 ^ s0 ^ (s1 >> 11) ^ (s0 >> 5); // b,c
	//s1 ^= s1 << 21; // a
	//state[*p] = s1 ^ s0 ^ (s1 >> 13) ^ (s0 >> 1); // b,c
	s1 ^= s1 << 25; // a
	state[*p] = s1 ^ s0 ^ (s1 >> 16) ^ (s0 >> 1); // b,c
	return res;
	//return state[*p];
}

static void xorshift256_next_1(uint64_t state[4], int p) {
	const uint64_t s0 = state[p];
	uint64_t s1 = state[p = (p + 1) & 3];
	s1 ^= s1 << 30; // a
	state[p] = s1 ^ s0 ^ (s1 >> 1) ^ (s0 >> 2); // b,c
}
static void xorshift256_next_4(uint64_t state[4]) {
	xorshift256_next_1(state, 0);
	xorshift256_next_1(state, 1);
	xorshift256_next_1(state, 2);
	xorshift256_next_1(state, 3);
}
